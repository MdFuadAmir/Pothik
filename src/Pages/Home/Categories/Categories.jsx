    ...
    return (
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
        {categories.map((cat, index) => (
          <div
            key={index}
            className="flex flex-col items-center justify-center text-center p-6 rounded-xl shadow-md hover:shadow-lg transition transform hover:scale-105 bg-linear-to-br from-gray-100 to-gray-200"
          >
            <div className="mb-4">{cat.icon}</div>
            <h3 className="text-xl font-semibold">{cat.name}</h3>
            <p className="text-gray-800 text-center text-sm">{cat.description}</p>
          </div>
        ))}
      </div>
    </section>
